...
mod tests {
    ...
    #[test]
    fn avatar_command_and_rpc_methods_are_exposed_by_category() {
        let env = avatar_env();
        install_send_reply_recorders(&env);
        env.define(
            Rc::from("msg"),
        ...
        assert!(eval_bool(
            "(not (method-member? :prop (car (cdr (get-prop \"reply-term:1\")))))",
            &env,
        ));
        assert!(eval_bool(
        ...
        assert!(eval_bool(
            "(method-member? :prop (car (cdr (get-prop \"reply-term:2\"))))",
            &env,
        ));
        assert!(eval_bool(
            "(method-member? :name (car (cdr (get-prop \"reply-term:2\"))))",
            &env,
        ));
        assert!(eval_bool(
            "(method-member? :describe (car (cdr (get-prop \"reply-term:2\"))))",
            &env,
        ));
        assert!(eval_bool(
    ...
    #[test]
    fn unowned_thing_without_recovery_secret_can_be_claimed() {
        let env = thing_env();
        env.define(
            Rc::from("msg"),
            Value::Msg(sample_msg("did:ma:owner", "did:ma:runtime#lamp")),
    ...
    #[test]
    fn avatar_conjure_builds_standard_init_for_inventory_parent() {
        let env = avatar_env();
        let did = "did:ma:did";
        let mut config = std::collections::HashMap::new();
        config.insert("runtime".to_string(), "did:ma:runtime".to_string());
        ...
        eval_all(
            r#"
            (set-prop! "did" "did:ma:did")
            (set-prop! "inventory" "did:ma:runtime#satchel")
            (define (ma-create-actor kind behaviour init fragment)
              (set-prop! "create-kind" kind)
        ...
        assert_eq!(
            eval_str("(get-prop \"create-kind\")", &env),
            "/ma/thing/0.0.1"
        );
        assert_eq!(
            eval_str("(get-prop \"create-init\")", &env),
            "(set-init-prop! \"name\" \"Brass Lantern\")\n(set-init-prop! \"owner\" \"did:ma:did\")\n(set-init-prop! \"parent\" \"did:ma:runtime#satchel\")\n(ma-save-state!)\n"
        );
        assert_eq!(
            eval_all("(get-prop \"reply-term:1\")", &env).unwrap(),
            Value::list(vec![Value::symbol(":ok"), Value::str("did:ma:runtime#new-thing")])
        );
        assert_eq!(
            eval_all("(get-prop \"sent-term:1\")", &env).unwrap(),
            Value::list(vec![
                Value::symbol(":print"),
                Value::str("Conjure requested: did:ma:runtime#new-thing."),
            ])
        );
    }

    #[test]
    fn avatar_conjure_requires_named_syntax() {
        let env = avatar_env();
        let did = "did:ma:did";
        let mut config = std::collections::HashMap::new();
        config.insert("runtime".to_string(), "did:ma:runtime".to_string());
    ...
    #[test]
    fn avatar_conjure_agent_uses_scheme_agent_kind() {
        let env = avatar_env();
        let did = "did:ma:did";
        let mut config = std::collections::HashMap::new();
        config.insert("runtime".to_string(), "did:ma:runtime".to_string());
    ...
    #[test]
    fn avatar_targeted_property_commands_resolve_multi_word_visible_names() {
        let env = avatar_env();
        let did = "did:ma:did";
        let mut config = std::collections::HashMap::new();
        config.insert("runtime".to_string(), "did:ma:runtime".to_string());
        ...
        env.define(
            ...
            Value::Msg(sample_term_msg(
                ...
                "did:ma:runtime#avatar",
                Value::list(vec![
                    Value::symbol(":prop"), Value::str("brass"), Value::str("lamp"),
                    Value::str("description"), Value::str("Bright"), Value::str("and"), Value::str("warm"),
                ]),
            )),
        );
        ...
        assert_eq!(
            eval_all("(get-prop \"sent-term:1\")", &env).unwrap(),
            Value::list(vec![Value::symbol(":prop"), Value::str("description"), Value::str("Bright"), Value::str("and"), Value::str("warm")]),
        );
        ...
        env.define(
            ...
            Value::Msg(sample_term_msg(
                ...
                "did:ma:runtime#avatar",
                Value::list(vec![
                    Value::symbol(":name"), Value::str("brass"), Value::str("lamp"), Value::str("as"),
                    Value::str("Brass"), Value::str("Lantern"), Value::str("with"), Value::str("nick"), Value::str("lantern"),
                ]),
            )),
        );
        eval_all("(on-message name_msg)", &env).unwrap();
        assert_eq!(eval_str("(get-prop \"sent-target:2\")", &env), "did:ma:runtime#lamp");
        assert_eq!(eval_all("(get-prop \"sent-term:2\")", &env).unwrap(), Value::list(vec![Value::symbol(":prop"), Value::str("name"), Value::str("Brass"), Value::str("Lantern")]));
        assert_eq!(eval_all("(get-prop \"sent-term:3\")", &env).unwrap(), Value::list(vec![Value::symbol(":prop"), Value::str("nick"), Value::str("lantern")]));

        env.define(
            Rc::from("describe_msg"),
            Value::Msg(sample_term_msg(
                did,
                "did:ma:runtime#avatar",
                Value::list(vec![Value::symbol(":describe"), Value::str("lamp"), Value::str("as"), Value::str("A"), Value::str("bright"), Value::str("lantern")]),
            )),
        );
        eval_all("(on-message describe_msg)", &env).unwrap();
        assert_eq!(eval_all("(get-prop \"sent-term:4\")", &env).unwrap(), Value::list(vec![Value::symbol(":prop"), Value::str("description"), Value::str("A"), Value::str("bright"), Value::str("lantern")]));
    }
    ...
    #[test]
    fn avatar_claim_with_target_token_routes_to_target_actor() {
        let env = avatar_env();
        install_send_reply_recorders(&env);
        let did = "did:ma:did";
        let mut config = std::collections::HashMap::new();
        ...
        env.define(
            ...
            Value::Msg(sample_term_msg(
                ...
                Value::list(vec![
                    Value::symbol(":claim"),
                    Value::str("did:ma:runtime#thing:"),
                ]),
            )),
        );
        eval_all("(on-message msg)", &env).unwrap();
        ...
        assert_eq!(
            eval_str("(get-prop \"sent-target:1\")", &env),
            "did:ma:runtime#thing"
        );
        assert_eq!(
            eval_all("(get-prop \"sent-term:1\")", &env).unwrap(),
            Value::list(vec![Value::symbol(":claim")]),
        );
        assert_eq!(
            eval_all("(get-prop \"reply-term:1\")", &env).unwrap(),
            Value::symbol(":ok")
    ...
    #[test]
    fn avatar_claim_with_target_token_forwards_secret_argument() {
        let env = avatar_env();
        install_send_reply_recorders(&env);
        let did = "did:ma:did";
        let mut config = std::collections::HashMap::new();
        ...
        assert_eq!(
            eval_all("(get-prop \"sent-term:1\")", &env).unwrap(),
            Value::list(vec![Value::symbol(":claim"), Value::str("hemmelig")]),
        );
    }

    #[test]
    fn avatar_claim_by_inventory_name_routes_to_target_actor() {
        let env = avatar_env();
        install_send_reply_recorders(&env);
        let did = "did:ma:did";
        let mut config = std::collections::HashMap::new();
        ...
        env.define(
            ...
            Value::Msg(sample_term_msg(
                did,
                "did:ma:runtime#avatar",
                Value::list(vec![Value::symbol(":claim"), Value::str("lamp")]),
            )),
        );
        eval_all("(on-message msg)", &env).unwrap();
    ...
    #[test]
    fn avatar_claim_without_target_token_still_routes_to_room() {
        let env = avatar_env();
        install_send_reply_recorders(&env);
        let did = "did:ma:did";
        let mut config = std::collections::HashMap::new();
    ...
    #[test]
    fn room_recycle_rejects_spoofed_delegated_did() {
        let env = room_env();
        let mut config = std::collections::HashMap::new();
        config.insert("runtime".to_string(), "did:ma:runtime".to_string());
        config.insert("self".to_string(), "did:ma:runtime#room".to_string());
        crate::state::set_config(config);
        install_send_reply_recorders(&env);
        let owner_avatar = eval_str("(avatar-fragment \"did:ma:owner\")", &env);
        let owner_avatar_did_url = format!("did:ma:runtime#{owner_avatar}");

        env.define(
            Rc::from("recycle_msg"),
            Value::Msg(sample_term_msg(
                &owner_avatar_did_url,
                "did:ma:runtime#room",
                Value::symbol(":recycle"),
            )),
        );
        ...
        eval_all(
                        &format!(
                                r#"
                                (test-avatar-claim-with-did! "{owner_avatar_did_url}" "did:ma:owner" "Owner")
                                (set-thing! "lamp" "did:ma:runtime#lamp")
                                (set-claim! "did:ma:runtime#lamp"
                                    (map-set
                                        (map-set
                                            ...
                                            "nick" "lamp")
                                        "description" "A brass lamp."))
                                ((find-method :recycle) (list "did:ma:intruder" "lamp") recycle_msg)
                                ((find-method :recycle) (list "did:ma:owner" "lamp") recycle_msg)
                                "#
                        ),
            &env,
        )
        ...
        assert_eq!(
            ...
            Value::list(vec![
                Value::symbol(":print"),
                Value::str("Unknown agent, thing, or container: did:ma:intruder"),
            ])
        );
        ...
        assert_eq!(
            eval_all("(get-prop \"sent-term:2\")", &env).unwrap(),
            Value::list(vec![Value::symbol(":recycle"), Value::str("did:ma:owner")])
        );
        ...
        assert_eq!(
            ...
            Value::list(vec![
                Value::symbol(":print"),
                Value::str("Recycle requested for lamp."),
            ])
        );
    }
    ...
    fn avatar_put_thing_transfers_visible_item_into_container() {
        ...
        fn avatar_recycle_routes_container_child_through_named_container() {
                ...
                .unwrap();
                    eval_all(
                    "(send-recycle (list \"did:ma:runtime#lamp\" \"in\" \"did:ma:runtime#bag\"))",
                        &env,
                    )
                    .unwrap();
    ...
    #[test]
    fn thing_owner_avatar_may_update_props() {
        let env = thing_env();
        install_send_reply_recorders(&env);
        let owner = "did:ma:owner";
        let mut config = std::collections::HashMap::new();
        ...
        .unwrap();
        let owner_avatar = format!(
            "did:ma:runtime#{}",
            eval_str(&format!(r#"(avatar-fragment "{owner}")"#), &env)
        );
        env.define(
            Rc::from("prop_msg"),
            Value::Msg(sample_term_msg(
                &owner_avatar,
                "did:ma:runtime#lamp",
                Value::list(vec![Value::symbol(":prop"), Value::str("name"), Value::str("lantern")]),
            )),
        );
        eval_all("(on-message prop_msg)", &env).unwrap();
        assert_eq!(eval_str("(name)", &env), "lantern");
        assert_eq!(eval_all("(car (get-prop \"reply-term:1\"))", &env).unwrap(), Value::symbol(":ok"));
    }
    ...
    #[test]
    fn inventory_forgets_thing_after_recycle_departure_ctx() {
        // Regression test: a thing that recycles while sitting in an avatar's
        // inventory container must be removed from that container's children
        // map, even though the departure ctx it sends carries an explicitly
        // empty "parent" (there is no new parent - the actor is terminating).
        let env = container_env();
        install_send_reply_recorders(&env);
        let mut config = std::collections::HashMap::new();
        config.insert("runtime".to_string(), "did:ma:runtime".to_string());
        ...
        .unwrap();
        env.define(Rc::from("lamp_ctx"), lamp_ctx.clone());
        eval_all("(remember-child! lamp_ctx)", &env).unwrap();
        assert_eq!(eval_str("(contents-text)", &env), "Contents:\nAladdins lampe = did:ma:runtime#lamp");

        let departure_ctx =
            eval_all(r#"(map-set lamp_ctx "parent" "")"#, &env).unwrap();
        env.define(
            Rc::from("lamp_departure_msg"),
            Value::Msg(sample_term_msg(
                "did:ma:runtime#lamp",
                "did:ma:runtime#inventory",
        ...
        eval_all("(on-message lamp_departure_msg)", &env).unwrap();

        assert!(eval_bool("(not (child-ctx \"did:ma:runtime#lamp\"))", &env));
        assert_eq!(eval_str("(contents-text)", &env), "Contents: none.");
        assert_eq!(
            eval_all("(get-prop \"reply-term:1\")", &env).unwrap(),
            Value::symbol(":ok")

...
(define (handle-node-roll-call! args msg)
  (cond ((not (or (roll-call-authorised? msg)
        ...
        ((equal? (car args) "begin")
           (let ((secs (if (null? (cdr args)) #f (car (cdr args)))))
             (roll-call-begin! msg (if (string? secs) (string->number secs) secs))))
        (else (roll-call-end! msg))))
(set-rpc-method! :roll-call handle-node-roll-call!)
